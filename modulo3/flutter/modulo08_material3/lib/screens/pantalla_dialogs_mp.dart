...
class PantallaDialogs extends StatelessWidget {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(esError
            ? 'Error: no se pudo procesar el rol de pagos'
            : 'Rol de pagos procesado correctamente'),
  ...
  Future<void> _mostrarConfirmacion(BuildContext context) async {
    final confirmar = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          '¿Estás seguro de que deseas eliminar a Gabriela Calderón?\n'
  ...
  Future<void> _mostrarFormulario(BuildContext context) async {
    ...
    if (ctrlNombre.text.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Empleado "${ctrlNombre.text}" agregado')),
      );
    }
  }
      ...
      body: ListView(
        ...
        children: [
          ...
          OutlinedButton.icon(
            ...
            label: const Text('Eliminar empleado (confirmación)'),
          ),
          const SizedBox(height: 8),
          FilledButton.tonal(
